...
//        (void) version();
  ...
  try {
    app = new apnspusher::App(prompt, (config) ? config : "etc/apnspusher.conf", console);
  } // try
  ...
  // write pid file
  if (!apnspusher::App::write_pid(apnspusher::App::kPidFile)) {
    std::cerr << "Could not write to PID file." << std::endl;
    exit(1);
  } // if

  elog.open("app", "logs/apnspusher.log");
